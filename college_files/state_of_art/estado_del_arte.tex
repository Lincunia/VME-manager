...
se cita en \textcite{Zambrano2020}, se puede sobreentender el mantenimiento
...
(Bishop, 2006  como se cita en \textcite{Janiesch2021})
...
\begin{itemize}
  ...
  \item \textbf{No supervisado:}
    ...
    (Witten et al., 2011, como se cita en \textcite{Singh2022})
  ...
  \item \textbf{Aprendizaje por refuerzo:}
    ...
    \begin{displayquote}
    El aprendizaje por refuerzo funciona secuencialmente en un entorno realizando
    una acción, evaluando su recompensa y ajustando las acciones siguientes en
    consecuencia. En concreto, un paradigma de aprendizaje por refuerzo implica un
    agente que observa el entorno y realiza acciones para maximizar la recompensa
    determinada por el problema en cuestión.
    (Richard S Sutton y Andrew G Barto., 2018, como se cita en
    \textcite{Chen2023})
    \end{displayquote}
...
(Esic Business \& Marketing School, 2020, como se cita en \textcite{Bueno2022})
...
\begin{displayquote}
  ...
  (Dudzinski DM, et al., 2010, como se cita en \textcite{Banja2020})
\end{displayquote}
...
como se cita en \textcite{Mosquera2019})
...
\begin{displayquote}
  ...
  (MinTic, 2021, como se cita en \textcite{Bueno2022})
\end{displayquote}

\subsubsection{Ciberespacio}
Este término se suele utilizar para entender la noción de ciberseguridad y que
se entiende como una serie de ``caminos'' en los que la información se manipula
y se transmite; se sobreentiende como un ``entorno'' en el que virtualmente la
misma se mueve, y la manera en como lo hace es a través de la red: ``Este se
define como el espacio artificial creado por el conjunto de sistemas de la
información y telecomunicaciones que utilizan las TIC, es decir de redes de
ordenadores, mucho más que Internet, más que los mismos sistemas y equipos''
(Sanchez, 2019, como se cita en \textcite{Bueno2022})

\subsubsection{Tipos de malware}
\textcite{Rosli2019} describen las características más esenciales y varios de
ellos funcionan de tal manera que se puedan realizar los objetivos de los
atacantes. 

% Ponte a ver el canal de VMalware para que completes esta madre XD
\begin{itemize}
  \item \textbf{Botnet:}
    % Un texto no serio evidentemente
    Un montón de equipos buscando cargarse lo que fuera de un servidor... no es un
    DDoS, sino más bien de una colección de equipos zombis tratando de hacer
    acciones malas.

    ``Botnet también se define como una colección de ordenadores que han sido
    infectados por software malicioso y los convierte en bots, drones o zombis, que
    se han integrado en una colección mayor a través de una infraestructura
    centralizada de mando y control.'' \parencite[e.g.,][]{Rosli2019}

  \item \textbf{Ransomware:}
    % Completarlo
    “Normalmente, una máquina infectada por ransomware queda ``congelada'', ya que
    el usuario no puede abrir ningún archivo, y la imagen del escritorio se utiliza
    para proporcionar información sobre las demandas de los atacantes.''
...
\begin{itemize}
  \item \textbf{Phishing:}
    ...
    \begin{displayquote}
      ...
      (INCIBE, 2010, como se cita en \textcite{Bueno2022})
    \end{displayquote}
